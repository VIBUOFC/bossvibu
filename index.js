...
function addMetadata(packname, author) {	
	if (!packname) packname = 'ShanBot'; if (!author) author = 'bossxvibu';	
...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: 'Un toque manin estoy procesando😎\n\n❗Por favor no hacer spam👏❗\n\n_*BO$$ VIBU*_',
				success: '✔️ Listo ✔️',
				...
				only: {
					...
					ownerB: '[❗] Este comando solo lo usa Vibu',
...
/******ENTRADA FIN DE FUNCIONES******/
			function addMetadata(packname, author) {	
				if (!packname) packname = 'BO$$ VIBU'; if (!author) author = 'VIBU#10';	
				author = author.replace(/[^a-zA-Z0-9]/g, '');	
				let name = `${author}_${packname}`
...
/******JUEGOS VIBU LA PUTA MADRE NO TE OLVIDES******/
		...
		if (!isUser) return reply(mess.only.daftarB)
			...
			case 'todos':
				client.updatePresence(from, Presence.composing) 
					...
					mentions('╔══✪〘 *TODOS* 〙✪══\n╠➥'+teks+'╚═〘 _*VIBU LEGEND*_ 〙', members_id, true)
					break
                                case 'send':
					var pc = body.slice(6)
			...
			//ANTILINKS DE REDES SOCIALES FLACO ACEPTALO SOLO LO ESTAS EDITANDO REALMENTE VIBU TE HIZO TODO ESTO	
				...
				//FIN DE ANTILINK HECHO POR VIBU
